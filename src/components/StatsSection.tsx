...
export const StatsSection = () => {
  ...
  return (
    <section ref={sectionRef} id="sobre" className="py-20 px-6 bg-gradient-to-b from-background to-card relative overflow-hidden">
      {/* Background Elements */}
      ...
      <div className={`max-w-7xl mx-auto relative`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {stats.map((stat, index) => (
            <div
              key={index}
              className={`group relative bg-card/50 backdrop-blur-sm rounded-2xl p-8 border border-border hover:border-primary/50 transition-all duration-1000 hover:shadow-glow hover:-translate-y-2 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                }`}
